...
public final class M2tsExtractor implements Extractor {
  ...
  private static final String TAG = "M2tsExtractor";

  private static final int TS_PACKET_SIZE = 188;
  private static final int TS_PACKET_HEADER_SIZE = 4;
  private static final int TS_SYNC_BYTE = 0x47; // First byte of each TS packet.
  private static final int TS_PAT_PID = 0;
  private static final int TS_PID_MASK = 0x1fff;
  ...
  public M2tsExtractor(PtsTimestampAdjuster ptsTimestampAdjuster) {
    this(ptsTimestampAdjuster, 0);
  }

  public M2tsExtractor(PtsTimestampAdjuster ptsTimestampAdjuster, int workaroundFlags) {
  ...
  public boolean sniff(ExtractorInput input) throws IOException, InterruptedException {
    ...
    input.advancePeekPosition(TS_PACKET_HEADER_SIZE); //skip header
    for (int i = 0; i < 5; i++) {
      input.peekFully(scratch, 0, 1);
      if ((scratch[0] & 0xFF) != 0x47) {
        return false;
      }
      input.advancePeekPosition(TS_PACKET_SIZE - 1 + TS_PACKET_HEADER_SIZE);
    }
    return true;
  }
  ...
  public int read(ExtractorInput input, PositionHolder seekPosition)
	...
	input.skip(TS_PACKET_HEADER_SIZE);
...
//    	  Log.i(TAG, "got reader: " +  payloadReader);
    	  payloadReader.consume(tsPacketBuffer, payloadUnitStartIndicator, output);
      }else{
    	  Log.i(TAG, "can not got reader of pid:" + Integer.toHexString(pid));
      }
    }
    ...
    public void consume(ParsableByteArray data, boolean payloadUnitStartIndicator,
      ...
      for (int i = 0; i < programCount; i++) {
        ...
        if (programNumber == 0) {
          ...
          Log.i(TAG, "Pat put reader pid: 0x" + Integer.toHexString(pid));
  ...
  private class PmtReader extends TsPayloadReader {
    ...
    public void consume(ParsableByteArray data, boolean payloadUnitStartIndicator,
      ...
      while (remainingEntriesLength > 0) {
        ...
        Log.i(TAG, "pmt steamtype: 0x" + Integer.toHexString(streamType));
        switch (streamType) {
        ...
        if (pesPayloadReader != null) {
          ...
          Log.i(TAG, "Pmt put reader pid: 0x" + Integer.toHexString(elementaryPid));
